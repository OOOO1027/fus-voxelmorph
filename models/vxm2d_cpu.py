...
class ConvBlock(nn.Module):
    """
    2D convolution + normalization + activation.
...
class Unet(nn.Module):
    ...
    def __init__(self, in_channels, enc_channels=(16, 32, 32, 32),
        ...
        # Skip channels come from the encoder output *before* each level,
        # traversed in reverse.  The skip for the i-th decoder level is the
        # output of encoder level (n_enc - 1 - i), i.e. enc_channels index
        # (n_enc - 2 - i) for i < n_enc-1, and the original input for the
        # last upsample level.
        ...
        for i in range(n_enc):
            ...
            self.up_convs.append(ConvBlock(ch + skip_ch, dec_channels[i], norm=norm))
            ch = dec_channels[i]

        # ---- extra full-resolution convolutions (no skip, no upsample) ----
        self.extra_convs = nn.ModuleList()
        for i in range(n_enc, len(dec_channels)):
            self.extra_convs.append(ConvBlock(ch, dec_channels[i], norm=norm))
            ch = dec_channels[i]
